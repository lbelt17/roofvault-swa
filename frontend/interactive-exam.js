...
(function () {
  ...
  function renderExam(container, rawText){
    ...
    const state=new Map(); // id -> {choice, tf, sa, revealed}
        ...
        container.append(
          el("div",{class:"card"},
            el("div", {style:{fontSize:"14px", color:"#334155"}},
              'Paste your generated text into the textarea or set window.ExamText. Expected lines like:',
...
Cites: ...`)
    ...
    // Source of exam text:
    // 1) If a textarea with id="exam-source" exists, use its value
    // 2) Else use window.ExamText (you can fill it from your generator)
    // 3) Else render an empty state
    const ta = document.getElementById("exam-source");
    function readText(){ return ta && (ta as HTMLTextAreaElement).value || (window as any).ExamText || ""; }

    // Controls
    const controls = el("div",{class:"row"},
      el("div",{style:{fontWeight:"600"}}, "Exam Text"),
      el("div",{},
        el("button",{class:"btn", onclick:()=>{ (window as any).ExamText = (window as any).ExamText || ""; rerender(); }}, "Use window.ExamText"),
        el("button",{class:"btn btn-solid", style:{marginLeft:"8px"}, onclick:()=>{ rerender(); }}, "Render")
      )
    );

    const wrapper=el("div",{class:"exam-wrap"});
    wrapper.append(controls);
    mount.appendChild(wrapper);

    function rerender(){ 
      let examRoot = document.getElementById("exam-root");
      if(!examRoot){ examRoot = el("div",{id:"exam-root"}); wrapper.appendChild(examRoot); }
      renderExam(examRoot, readText());
    }
